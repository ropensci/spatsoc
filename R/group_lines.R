#' Groups Lines
#'
#' \code{group_lines} groups rows into spatial groups by creating trajectories and grouping based on spatial overlap. The function accepts a \code{data.table} with relocation data, individual identifiers and a \code{threshold}. The relocation data is transformed into \code{SpatialLines} and overlapping \code{SpatialLines} are grouped. The \code{threshold} argument is used to specify the criteria for distance between lines. Relocation data should be in two columns representing the X and Y coordinates.
#'
...
#'
#' The \code{id}, \code{coords}, \code{sortBy} (and optional \code{timegroup} and \code{splitBy}) arguments expect the names of respective columns in \code{DT} which correspond to the individual identifier, X and Y coordinates, sorting, timegroup (generated by \code{group_times}) and additional grouping columns.
#'
...
#' The \code{projection} expects a \code{PROJ.4} character string (such as those available on \url{spatialreference.org}). \code{group_lines} expects planar coordinates (not unprojected latitude, longitude).
#'
#' The \code{threshold} must be provided in the units of the coordinates. The \code{threshold} can be equal to 0 if strict overlap is required, else it needs to be greater than 0. The coordinates must be planar coordinates (e.g.: UTM). In the case of UTM, a \code{threshold} = 50 would indicate a 50m distance threshold.
#'
...
#'
#' @return \code{group_lines} returns the input \code{DT} appended with a \code{group} column.
#'
