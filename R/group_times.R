...
group_times <- function(
  ...
  if (is.null(threshold)) {
    ...
    if (
      ...
      dtm <-
        DT[,
          cbind(.SD[[1]], data.table::IDateTime(.SD[[1]])),
          .SDcols = datetime
        ]
    ...
    if (grepl('hour', threshold)) {
      nHours <- data.table::tstrsplit(threshold, ' ', type.convert = TRUE)[[1]]
      ...
      dtm[
        data.table::hour(itime) %% nHours < (nHours / 2),
      ...
      nMins <- data.table::tstrsplit(threshold, ' ', type.convert = TRUE)[[1]]
      if (!is.integer(nMins)) {
        if (nMins %% 1 != 0) {
          warning(
            strwrap(
      ...
      dtm[
        data.table::minute(itime) %% nMins < (nMins / 2),
      ...
      dtm[,
        c('adjMinute', 'adjHour', 'adjDate') := .(
          minutes,
          data.table::hour(itime),
          idate
        )
      ]

      dtm[minutes == 60L, c('adjMinute', 'adjHour') := .(0L, adjHour + 1L)]

      dtm[
        adjHour == 24L,
        c('adjMinute', 'adjHour', 'adjDate') := .(0L, 0L, idate + 1)
      ]

      dtm[, timegroup := .GRP, by = c('adjMinute', 'adjHour', 'adjDate')]

      data.table::set(
        dtm,
        j = c('adjMinute', 'adjHour', 'adjDate'),
        value = NULL
      )
      data.table::set(dtm, j = c('idate', 'itime'), value = NULL)
        ...
        if (!(rangeday %% nDays == 0)) {
          warning(
            strwrap(
              ...
              initial = "",
              x = paste0(
                'the minimum and maximum days in
