...
#' `edge_dist` (optionally with directions measured by
#' `direction_step` and `edge_direction`), zone thresholds, labels
...
#'  "direction_dyad" from `edge_direction`) is made relative to the focal
#'  individual's movement direction (column "direction" from
#'  `direction_step`). The argument blind_volume expects a single value to
...
#'
#' The `edges` must be a `data.table`. If your data is a
#' `data.frame`, you can convert it by reference using
#' [data.table::setDT()] or by reassigning using
#' [data.table::data.table()].
#'
#' @param edges distance based edge-list generated by `edge_dist`,
#'   optionally with direction columns from `direction_step` and
#'   `edge_direction` (see Details)
...
#'   (-2, 2), see Details
#'
#' @return `edge_zones` returns the input `edges` appended with a zone
