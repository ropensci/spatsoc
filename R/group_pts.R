#' Group Points
#'
#' \code{group_pts} groups rows into spatial groups. The function accepts a \code{data.table} with relocation data, individual identifiers and a threshold argument. The threshold argument is used to specify the criteria for distance between points which defines a group. Relocation data should be in two columns representing the latitude and longitude.
#'
#' The \code{id}, \code{coords} (and optionally \code{timegroup} and \code{splitBy}) arguments expect the names of a column in \code{DT} which correspond to the individual identifier, latitude and longitude, timegroup (generated by \code{group_times}) and additional grouping columns.
#'
...
#'
#' @return \code{group_pts} returns the input \code{DT} appended with a \code{group} column. This column represents the spatial (and if \code{timegroup} was provided - spatiotemporal) group. As with \code{\link{group_times}}, the actual value of \code{group} is meaningless, rather the contents of the group. If the data was reordered, the \code{group} may change, but the contents of each \code{group} would not.
#'
