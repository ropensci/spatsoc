...
{spatsoc} v 0.2.13 provides a geometry interface and internal functions that
...
expectation is that the "geometry" column is `sfc_POINT` representing each
relocation.
...
An example of a potential issue with intermixing packages between spatsoc functions
is that attributes
such as the sf column's bbox aren't automatically updated when subsetting with
data.table. This impacts the bbox returned, and also the bbox used for plotting. 
The recommendation is to run all required {spatsoc} functions then convert to
standard spatial formats as described below for further processing and visualization. 
