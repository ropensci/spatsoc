...
relocations. It can be used to build proximity-based social networks using
gambit-of-the-group format and edge-lists. In addition, the randomization
...
`spatsoc` leverages `data.table` to modify by reference and iteratively work on
...
See [here](https://github.com/r-spatial/sf/?tab=readme-ov-file#installing) for
help installing `sf`, see [here](https://github.com/r-quantities/units/?tab=readme-ov-file#installation) 
for help installing `units` and [here](https://github.com/r-spatial/lwgeom?tab=readme-ov-file#installing) for
...
These are then provided to the function using the names of the column in the
input data.
...
* multi-day blocks are consistent **across years** and timegroups from these
are **by year**. 
...
The main column returned by `group_times` is "timegroup". It represents the
...
The extra columns are provided to help the user investigate, troubleshoot and
interpret the timegroup.
...
The input `data.table`. It will returned with a column named group appended,
...
The coordinates must be planar, such as UTM (of whichever zone your relocations
are in).
...
If `area = FALSE`, see [3.2.1](#dt-1). If `area = TRUE`, the DT will not be
...
* `timegroup`: column name of time group
* `splitBy`: (optional) column names of extra variables to group on
* `fillNA`: logical indicating if NAs should be returned for individuals that
...
* `timegroup`: column name of time group
* `splitBy`: (optional) column names of extra variables to group on
* `threshold`: (optional) spatial distance threshold to set maximum distance 
between an individual and their neighbour.

This function can be used to generate edge-lists defined either by nearest
neighbour or nearest neighbour with a maximum distance. NAs are returned for
nearest neighbour for an individual was alone in a timegroup (and/or splitBy) or
if the distance between an individual and it's nearest neighbour is greater than
the threshold.
...
After using the grouping functions, we can determine the number of individuals 
in a temporal or spatial group. 
