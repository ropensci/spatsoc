...
`spatsoc` can be used in social network analysis to generate gambit of the group format data from GPS relocation data and perform data stream randomization. 
...
The following code chunk showing `get_gbi` can be used for outputs from any of `group_pts`, `group_lines` or `group_polys(area = FALSE)`. For the purpose of this vignette however, we will consider the outputs from `group_pts` for the following code chunk (3. `get_gbi`).
...
```{r}
## Create a data.table of unique combinations of iteration and year
cols <- c('iteration', 'yr')
iterYearLs <- randStep[, do.call(unique, list(x = .SD)), .SDcols = cols]
...
cols <- c('iteration', 'yr', 'population')
iterYearLs <- randDaily[, do.call(unique, list(x = .SD)), .SDcols = cols]
...
gbiLs <- mapply(FUN = function(i, y, p) {
  get_gbi(randDaily[iteration == i & yr == y & population == p],
          'group', 'randomID')
  },
  i = iterYearLs$iter,
  y = iterYearLs$yr,
...
## Create a data.table of unique combinations of iteration and year
cols <- c('iteration', 'yr')
iterYearLs <- randTraj[, do.call(unique, list(x = .SD)), .SDcols = cols]
...
To calculate observed network metrics, use the network (`net`) produced in 4. `asnipe::get_network` from 2016 data.
...
With the list of networks, we can generate a list of graphs with `igraph::graph.adjacency` (for example) and calculate network metrics. 
...
meanMets <- out[, lapply(.SD, mean), by = .(ID, yr)]
```
