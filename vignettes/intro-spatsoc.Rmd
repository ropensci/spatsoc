...
pt.grps <- GroupPts(sublocs, idField = 'ID', distanceThreshold = 50,
...
### `GroupPts`
The `GroupPts` function compares the relocations of all individuals at each time step and groups individuals based on a distance threshold provided by the user.
...
GroupPts(locs, 50, 'datetime', '5 minutes',
...
### `GroupLines`
The `GroupLines` function groups individuals whose trajectories intersect in a specified time interval (eg: daily). This represents a coarser grouping method than `GroupPts` which can help understand shared space at daily, weekly or other temporal resolutions.
