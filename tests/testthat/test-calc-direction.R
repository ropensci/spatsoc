...
test_that('units are returned', {
  expect_s3_class(
    DT[, calc_direction(geometry_longlat, use_transform = FALSE)],
    'units'
  )
  expect_identical(
    DT[,
      units(
        calc_direction(geometry_longlat, use_transform = FALSE)
      )$numerator
    ],
    'rad'
  )
...
test_that('expected range returned', {
  ...
  expect_equal(
    DT[, min(calc_direction(geometry_longlat, use_transform = FALSE))],
    -y,
    tolerance = 0.1
  )
  expect_equal(
    DT[, max(calc_direction(geometry_longlat, use_transform = FALSE))],
    y,
    tolerance = 0.1
  )

  expect_equal(
    DT[, min(calc_direction(
      x_a = X_longlat,
      y_a = Y_longlat,
      crs = crs_longlat,
      use_transform = FALSE
...
test_that('NAs returned as expected', {
  ...
  expect_error(
    XY_NA[, calc_direction(geometry, use_transform = FALSE)],
    'missing values in coordinates'
  )
  expect_error(
    XY_NA[, calc_direction(
  ...
  expect_error(
    geometry_NA[, calc_direction(geometry_longlat_miss)],
    'missing values in coordinates'
  )
  expect_error(
    geometry_NA[, calc_direction(geometry_longlat, geometry_longlat_miss)],
    'missing values in coordinates'
  )
})
