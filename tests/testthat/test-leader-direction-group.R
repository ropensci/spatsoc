...
test_that('arguments required, otherwise error detected', {
  expect_error(
    leader_direction_group(DT, coords = coords, return_rank = NULL, crs = utm),
    'return_rank must be'
  )
  expect_error(
    leader_direction_group(DT, coords = coords, group = NULL, crs = utm),
    'group must be'
  )

  expect_error(
    leader_direction_group(DT, coords = coords, group = NULL, crs = NULL),
    'crs must be'
  )
  ...
  expect_message(leader_direction_group(DT_geo, crs = 4326), 'ignored')
})

test_that('column names must exist in DT', {
  expect_error(
    leader_direction_group(DT, coords = rep('potato', 2)),
    'potato field'
  )
  ...
  expect_error(
    leader_direction_group(DT, coords = coords, crs = utm, group = 'potato'),
    'group_pts'
  )

  # geo
  expect_error(leader_direction_group(DT_geo, geometry = 'potato'), 'potato')
})


test_that('coords are correctly provided or error detected', {
  expect_error(
    leader_direction_group(DT, coords = c('X', NULL), crs = utm),
  ...
  # geo
  expect_error(leader_direction_group(DT_geo, geometry = 'X'), 'geometry')
})

test_that('radians expected else error', {
  copyDT <- copy(DT)[,
    group_direction := units::set_units(group_direction, 'degree')
  ]
  expect_error(
    leader_direction_group(copyDT, coords = coords, crs = utm),
    'direction_group'
  )
})
...
test_that('position_group_direction column succesfully detected', {
  ...
  expect_message(leader_direction_group(copy_DT), 'position_group_direction')
})
...
test_that('no rows are added to the result DT', {
  ...
  expect_equal(nrow(copy_DT), nrow(leader_direction_group(copy_DT)))
})
...
test_that('1 or 2 (return_rank = TRUE) column(s) added to the result DT', {
  ...
  expect_equal(
    ncol(copy_DT) + 2,
    ncol(leader_direction_group(copy_DT, return_rank = TRUE, group = 'group'))
  )
})
...
test_that('column(s) added to the result DT are expected range', {
  ...
  expect_equal(
    leader_direction_group(DT, coords = coords, crs = utm, group = 'group')[
      rank_position_group_direction < 0,
      .N
    ],
    0
  )
...
test_that('ud_units must be m', {
  expect_error(
    leader_direction_group(DT, coords = coords, group = 'group', crs = 4326),
    'planar'
  )

  expect_error(
    leader_direction_group(DT_geo, geometry = 'geometry_longlat'),
    'planar'
  )
})
