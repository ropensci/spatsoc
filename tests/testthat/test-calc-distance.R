...
dest_coords_longlat <- paste0('dest_', coords, '_longlat')
DT[,
  (dest_coords_longlat) := as.data.table(sf::st_coordinates(
    dest_geometry_longlat
  ))
]
...
test_that('units are returned when use_dist is FALSE else numeric', {
  expect_s3_class(
    DT[, calc_distance(geometry_longlat, use_dist = FALSE)],
    'units'
  )
  expect_s3_class(
    DT[, calc_distance(
      geometry_longlat,
      st_shift_longitude(geometry_longlat),
      use_dist = FALSE
    )],
    'units'
  )
...
test_that('expected dims returned', {
  N <- 100
  expect_length(
    DT[seq.int(N), calc_distance(geometry_longlat, use_dist = FALSE)],
    N * N
  )
  expect_length(
    DT[
      seq.int(N),
      calc_distance(geometry_longlat, dest_geometry_longlat, use_dist = FALSE)
    ],
    N
  )
  ...
  expect_length(DT[seq.int(N), calc_distance(geometry, use_dist = TRUE)], N * N)
  expect_length(
    DT[seq.int(N), calc_distance(geometry, geometry[1], use_dist = TRUE)],
    N
  )
...
test_that('expected range returned', {
  expect_gte(DT[, min(calc_distance(geometry, use_dist = TRUE))], 0)
  expect_gte(
    DT[, min(calc_distance(x_a = X, y_a = Y, crs = crs, use_dist = TRUE))],
    0
  )
  expect_gte(
    DT[, min(calc_distance(
      geometry,
      geometry_b = dest_geometry,
      use_dist = FALSE
    ))],
    units::set_units(0, 'm')
  )
...
test_that('NAs returned as expected', {
  ...
  expect_length(res, nrow(X_NA) * nrow(X_NA))
  expect_true(any(is.na(X_NA$X_longlat)))
  # expect_true(any(is.na(res)))
