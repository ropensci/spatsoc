...
test_that('DT is required', {
  expect_error(
    group_times(DT = NULL, datetime = NULL, threshold = '10 minutes'),
    'DT must be provided'
  )
})
...
test_that('time field correctly provided or error detected', {
  expect_error(
    group_times(DT, datetime = NULL, threshold = '10 minutes'),
    'datetime must be'
  )

  expect_error(
    group_times(DT, datetime = 'potato', threshold = '10 minutes'),
...
test_that('threshold properly provided', {
  ...
  expect_message(
    group_times(copyDT, datetime = 'datetime', threshold = NULL),
  ...
  copyDT <- copy(DT)[, datetime := as.POSIXct(datetime)]
  expect_error(
    group_times(copyDT, datetime = 'datetime', threshold = '50 potato'),
...
test_that('time field is appropriate format', {
  ...
  expect_error(
    group_times(copyDT, datetime = 'datetimenumeric', threshold = '60 minutes'),
    'time field provided must be either',
    fixed = FALSE
  )
})
...
test_that('threshold provided must be in units of hours, minutes, days', {
  copyDT <- copy(DT)[, datetime := as.POSIXct(datetime)]
  expect_error(
    group_times(copyDT, datetime = 'datetime', threshold = '13 potatoes'),
    'must provide threshold in units',
    fixed = FALSE
  )
})
...
test_that('check that 60 minutes and 1 hour are the same result', {
  ...
  expect_equal(
    {
      copyDT <- copy(DT)[, datetime := as.POSIXct(datetime)]
      group_times(copyDT, datetime = 'datetime', threshold = '1 hour')
    },
    {
      copyDT <- copy(DT)[, datetime := as.POSIXct(datetime)]
...
test_that('warns if block is not even', {
  ...
  if (
    copyDT[,
      max(data.table::yday(datetime)) -
        min(data.table::yday(datetime))
    ] %%
      13 ==
      0
  ) {
  ...
  expect_warning(
    group_times(copyDT, datetime = 'datetime', threshold = blockLength),
    'the minimum and maximum days',
    fixed = FALSE
  )
})
...
test_that('multiyear blocks are well handled', {
  ...
  expect_equal(
    group_times(copyDT, datetime = 'isoDate', threshold = '1 day')[,
      .N,
      by = timegroup
    ]$N,
    copyDT[, .N, by = .(data.table::yday(isoDate))]$N
  )
  ...
  expect_equal(
    ...
    copyDT[, .N, by = .(data.table::yday(isoDate), data.table::year(isoDate))]$N
  )
})
...
test_that('group_times nearest hour with minutes threshold', {
  ...
  expect_equal(
    group_times(DT, c('idate', 'itime'), threshold = '5 minutes')[, uniqueN(
      timegroup
    )],
    3
  )
...
test_that('timegroups are based off years but blocks are consistent across all years', {
  ...
  blocks <- copyDT[, as.list(range(yday(datetime))), .(block, year(datetime))]
  setkey(blocks, V1, V2)
  expect_true(
