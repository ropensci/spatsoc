...
test_that('arguments required, otherwise error detected', {
  expect_error(direction_group(DT, group = NULL), 'group must be provided')
  expect_error(
    direction_group(DT, direction = NULL),
    'direction must be provided'
  )
})

test_that('column names must exist in DT', {
  expect_error(direction_group(DT, direction = 'potato'), 'potato field')
  expect_error(direction_group(DT, group = 'potato'), 'potato field')
})
...
test_that('no rows are added to the result DT', {
  ...
  expect_equal(nrow(copyDT), nrow(direction_group(copyDT)))
})

test_that('one column added to the result DT', {
  copyDT <- copy(clean_DT)

  expect_equal(ncol(copyDT) + 1, ncol(direction_group(DT)))
})
