...
test_that('column names must exist in DT', {
  expect_error(
    direction_to_centroid(DT, coords = rep('potato', 2), crs = utm),
    'potato field'
  )
...
test_that('coords are correctly provided or error detected', {
  expect_error(
    direction_to_centroid(DT, coords = c('X', NULL), crs = utm),
    'coords must be length 2'
  )
  expect_error(
    direction_to_centroid(DT, coords = c('X', 'ID'), crs = utm),
...
test_that('column added to the result DT is a double with units rad', {
  expect_type(
    direction_to_centroid(DT, coords = coords, crs = utm)$direction_centroid,
    'double'
  )
  expect_equal(
    units(
...
test_that('returns a data.table', {
  expect_s3_class(
    direction_to_centroid(DT, coords = coords, crs = utm),
    'data.table'
  )
})
...
test_that('use_transform errors if crs not provided', {
  ...
  get_geometry(
    copyDT,
    coords = paste0('centroid_', coords),
    crs = utm,
...
test_that('if coords null, geometry required', {
  expect_error(direction_to_centroid(DT, crs = utm), 'get_geometry?')

  get_geometry(DT, coords = coords, crs = utm)
  expect_error(direction_to_centroid(DT, crs = utm), 'centroid_group?')

  copy_DT <- copy(DT)
  copy_DT[, centroid := 42]
  expect_error(direction_to_centroid(copy_DT, crs = utm), 'sfc_POINT')
})
...
test_that('NAs in coordinates return NA', {
  ...
  expect_equal(
    copyDT[is.na(X), .N],
    direction_to_centroid(copyDT, coords = coords, crs = utm)[is.na(X)][
      is.na(direction_centroid),
      .N
    ]
  )
  ...
  expect_equal(
    copyDT[is.na(Y), .N],
    direction_to_centroid(copyDT, coords = coords, crs = utm)[is.na(Y)][
      is.na(direction_centroid),
      .N
    ]
  )
