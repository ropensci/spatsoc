...
test_that('arguments as expected, otherwise error/message/warn', {
  ...
  expect_error(
    distance_to_centroid(DT, coords = coords, group = NULL, return_rank = TRUE),
...
test_that('column names must exist in DT', {
  expect_error(
    distance_to_centroid(DT, coords = rep('potato', 2)),
    'potato field'
  )
  expect_error(
    distance_to_centroid(
...
test_that('coords/geometry are correctly provided or error detected', {
  expect_error(
    distance_to_centroid(DT, coords = c('X', NULL)),
    'coords must be length 2'
  )
  copy_DT <- copy(DT)[, X := as.character(X)]
  expect_error(
    distance_to_centroid(copy_DT, coords = coords),
  ...
  # geometry
  expect_error(distance_to_centroid(DT, geometry = 'potato'), 'get_geometry')
  expect_message(distance_to_centroid(DT, crs = utm), 'crs argument is ignored')
...
test_that('no rows are added to the result DT', {
  ...
  # geometry
  copyDT <- copy(clean_DT)
  expect_equal(nrow(copyDT), nrow(distance_to_centroid(copyDT)))
})
...
test_that('1/2 columns added to the result DT (depending on return_rank)', {
  ...
  expect_equal(
    ncol(copyDT) + 2,
    ncol(distance_to_centroid(copyDT, coords = coords, return_rank = TRUE))
  )

  copyDT <- copy(clean_DT)
  expect_equal(
    ncol(copyDT) + 1,
    ncol(distance_to_centroid(copyDT, coords = coords, return_rank = FALSE))
  )
...
test_that('column added to the result DT is a double', {
  # coords
  expect_type(
    distance_to_centroid(DT, coords = coords)$distance_centroid,
    'double'
  )

  expect_type(
    distance_to_centroid(DT, coords = coords, crs = utm)$distance_centroid,
    'double'
  )
...
test_that('message if overwritting rank_distance_centroid', {
  ...
  expect_message(
    distance_to_centroid(copy_DT, group = group, return_rank = TRUE),
    'rank_distance_centroid'
  )
})
...
test_that('NA for centroid returns NA for distance', {
  ...
  expect_all_true(
    distance_to_centroid(copy_DT, geometry = 'geometry_longlat')[
      sf::st_is_empty(geometry_longlat),
      is.na(distance_centroid)
    ]
  )
