...
test_that('columns must exist in edges', {
  ...
  expect_error(edge_zones(rm_col, zone_thresholds, zone_labels), 'distance')

  rm_col <- copy(dyad_directions)[, .SD, .SDcols = -'direction']
  expect_error(
    edge_zones(rm_col, zone_thresholds, zone_labels, blind_volume),
    'direction'
  )
...
test_that('columns are correctly provided or error detected', {
  ...
  expect_error(
    edge_zones(copyEdges, zone_thresholds, zone_labels),
    'distance must be of class numeric'
  )

  copyDyad <- copy(dyad_directions)[,
    direction := units::set_units(direction, 'degrees')
  ]
  expect_error(
    edge_zones(copyDyad, zone_thresholds, zone_labels, blind_volume),
    'radians',
  )

  copyDyadDir <- copy(dyad_directions)[,
    direction_dyad := units::set_units(direction_dyad, 'degrees')
  ]
  expect_error(
    edge_zones(copyDyadDir, zone_thresholds, zone_labels, blind_volume),
    'radians'
  )
})
...
test_that('one column added to the result is a factor', {
  ...
  expect_s3_class(
    edge_zones(copyEdges, zone_thresholds, zone_labels)$zone,
    'factor'
  )

  expect_equal(
    levels(edge_zones(copyEdges, zone_thresholds, zone_labels)$zone),
    zone_labels
  )
  ...
  expect_s3_class(
    edge_zones(copyDyadDirs, zone_thresholds, zone_labels, blind_volume)$zone,
    'factor'
  )
...
test_that('returns a data.table', {
  expect_s3_class(edge_zones(edges, zone_thresholds, zone_labels), 'data.table')
})


test_that('results are expected', {
  expected <- merge(
    edge_zones(edges, zone_thresholds, zone_labels),
  ...
  expected <- merge(
    edge_zones(dyad_directions, zone_thresholds, zone_labels, blind_volume),
    data.table(threshold = zone_thresholds, zone = zone_labels),
    by = 'zone'
  )
